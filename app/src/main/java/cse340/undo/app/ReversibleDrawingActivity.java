...
public class ReversibleDrawingActivity extends AbstractReversibleDrawingActivity {
    ...
    AbstractColorPickerView.ColorChangeListener colorChangeListener;
    ...
    /** List of menu item FABs for eraser menu. */
    @IdRes
    private static final int[] ERASE_MENU_ITEMS = {
            R.id.fab_erase_thick
    };

    /** State variables used to track whether menus are open. */
    private boolean isThicknessMenuOpen;
    private boolean isColorMenuOpen;
    private boolean isEraserMenuOpen;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        colorChangeListener = new AbstractColorPickerView.ColorChangeListener() {
            @Override
            public void onColorSelected(int color) {
                doAction(new ChangeColorAction(color));
            }
        };
        colorPicker.addColorChangeListener(colorChangeListener);
        ...
        findViewById(R.id.color_menu).setOnClickListener((v) -> {
            enableCollapsibleMenu(R.id.fab_thickness, THICKNESS_MENU_ITEMS, isColorMenuOpen);
            enableCollapsibleMenu(R.id.fab_erase, ERASE_MENU_ITEMS, isColorMenuOpen);
        ...
        draw.setOnTouchListener((view, event) -> {
            if (isThicknessMenuOpen) {
                ...
                return true;}
            else if(isEraserMenuOpen){
                isEraserMenuOpen = toggleMenu(ERASE_MENU_ITEMS, isEraserMenuOpen);
                enableCollapsibleMenu(R.id.fab_color, COLOR_MENU_ITEMS, !isEraserMenuOpen);
                colorPicker.mCurrentColor=Color.WHITE;
                return true;
            } else if (isColorMenuOpen) {
                isColorMenuOpen = toggleMenu(COLOR_MENU_ITEMS, isColorMenuOpen);
                enableCollapsibleMenu(R.id.fab_thickness, THICKNESS_MENU_ITEMS, !isColorMenuOpen);
                enableCollapsibleMenu(R.id.eraser_menu, ERASE_MENU_ITEMS, !isColorMenuOpen);
                return true;
            } else {
        ...
        addCollapsableMenu(R.layout.erase, ConstraintSet.BOTTOM, ConstraintSet.END, ERASE_MENU_ITEMS, this::onEraserMenuSelected);
    ...
    protected void onDestroy() {
        ...
        colorPicker.removeColorChangeListener(colorChangeListener);  // todo deregister the color change listener
    }
    ...
    private void onAction(AbstractReversibleAction action) {
        if (action instanceof ChangeColorAction) {
            ...
            colorPicker.setColor(currColor);  // TODO: update the color of the color picker if needed
        }
    }
    ...
    private void onEraserMenuSelected(View view) {
        switch (view.getId()) {
            case R.id.fab_erase_thick:
                doAction(new ChangeThicknessAction(50));
                doAction(new ChangeColorAction(Color.WHITE));
                break;
        }
